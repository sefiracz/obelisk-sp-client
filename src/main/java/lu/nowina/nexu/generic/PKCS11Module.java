 ...
 * {@link PKCS11Module#findCertificateWithNonRepudiationCritical()}for details.
 *
 * @author Roberto Resoli
 */
public class PKCS11Module { // TODO - kompletne prepsat a vykuchat (momentalne castecne prevzate z j4sign pod GPL, coz neni koser)
   ...
   */
  private String cryptokiLibrary = null;
  ...
  public PKCS11Module(String cryptokiLib, String reader) throws IOException, TokenException {
    this(cryptokiLib);
    initializeTokenInReader(reader);
  }

  public PKCS11Module(String cryptokiLib, long mechanism, String reader) throws IOException, TokenException {
    this(cryptokiLib, reader);
    setMechanism(mechanism);
    initializeTokenInReader(reader);
  }

  public PKCS11Module(String cryptokiLib) throws IOException, TokenException {
    super();
  ...
  private void initializeTokenInReader(String reader) throws
    ...
    for (int i = 0; i < tokens.length; i++) {
      ...
      // TODO wtf ?
      String readerFromPKCS11 = getSlotDescription(tokens[i]);
      ...
      if ((readerFromPKCS11.startsWith(reader)) ||
        ...
        log.debug("Setting token " + getSlotDescription(tokens[i]));
        tokenHandle = tokens[i];
      }
    }

    if (tokenHandle >= 0) {
      log.debug("Token handle initialized: " + tokenHandle);
    }
  }
  ...
  public List<String> getSignatureKeyLabels() throws PKCS11Exception {
    ...
    pkcs11Module.C_FindObjectsInit(getSession(), attributeTemplateList, true);
    long[] availableSignatureKeys = pkcs11Module.C_FindObjects(getSession(), 100);

    if (availableSignatureKeys == null) {
      log.debug("null returned - no signature key found");
    }
    else {
      log.debug("found " + availableSignatureKeys.length + " signature keys, picking first.");
      for (long signatureKeyHandle : availableSignatureKeys) {
        keyLabels.add(getKeyLabel(signatureKeyHandle, getSession()));
      }
    }
  ...
  private String getKeyLabel(long signatureKeyHandle, long sessionHandle) throws PKCS11Exception {
    CK_ATTRIBUTE[] template = { new CK_ATTRIBUTE() };
    template[0].type = PKCS11Constants.CKA_LABEL;
    pkcs11Module.C_GetAttributeValue(sessionHandle, signatureKeyHandle, template, true);
    return new String((char[]) template[0].pValue);
  }
   ...
   * given textual label.
   *
   * @param label the string label to search.
   * @return the integer identifier of the private key, or -1 if no key was
   * found.
   * @throws PKCS11Exception
   */
  public long getKeyWithLabel(String label) throws PKCS11Exception {
    ...
    if (availableSignatureKeys == null) {
      ...
      log.debug("found " + availableSignatureKeys.length
  ...
  /**
   * Sign the provided data with.
   *
   * @param keyLabel           label of the private key to use for signing.
   * @param digest             the digest of data to sign.
   * @return a byte[] containing signed data.
   * @throws IOException
   * @throws PKCS11Exception
   */
  public byte[] sign(String keyLabel, byte[] digest) throws IOException, PKCS11Exception {
    byte[] signature = null;
    if (getSession() < 0) {
      return null;
    }

    long signatureKeyHandle = getKeyWithLabel(keyLabel);
    log.debug("Start sign operation...");
    pkcs11Module.C_SignInit(getSession(), this.signatureMechanism, signatureKeyHandle, true);

    if (digest.length > 0 && digest.length < 1024) {
      log.debug("Signing digest");
      signature = pkcs11Module.C_Sign(getSession(), digest);
      log.debug("Signing finished");
    }
    else {
      log.debug("Digest length error");
    }
   ...
   */
  public long findCertificateWithNonRepudiationCritical(boolean allowExpired) throws TokenException, CertificateException {
    ...
    if (availableCertificates == null) {
      log.debug("null returned - no certificate key found");
    }
    else {
      log.debug("found " + availableCertificates.length
          + " certificates");
      ...
      for (int i = 0; (i < availableCertificates.length)
        ...
        log.debug("Checking KeyUsage for certificate with handle: "
   ...
   * subscription. See
   * {@link PKCS11Module#findCertificateWithNonRepudiationCritical()}.
   *
   * @see PKCS11Module#findCertificateWithNonRepudiationCritical()
   *
   * @param token ID of the token to query for the certificate.
   * @return the handle of the required certificate, if found; -1 otherwise.
   * @throws TokenException
   * @throws CertificateException findCertificateWithNonRepudiationCritical()
   */
  ...
  public long findCertificateWithNonRepudiationCritical(long token) throws
    ...
    if (s == -1L) {
      log.debug("Unable to open a session on token with handle: "
          + token);
      return -1L;
    }

    log.debug("finding a certificate with Critical KeyUsage including non repudiation on token with handle: " + token);
    ...
    pkcs11Module.C_FindObjectsInit(s, attributeTemplateList, true);
    ...
    if (availableCertificates == null) {
      ...
      log.debug("found " + availableCertificates.length + " certificates");
...
//  }
   ...
   *
   * @param javaCert the certificate to check as java object.
  ...
  public long findCertificateFromLabel(char[] label) throws PKCS11Exception {
    ...
    log.debug("find certificate from label.");
    ...
    //maximum of 100 at once
    if (availableCertificates == null) {
      log.debug("null returned - no certificate found");
    }
    else {
      log.debug("found " + availableCertificates.length
  ...
  public long findCertificateFromID(byte[] id) throws PKCS11Exception {
    ...
    log.debug("find certificate from id.");
   ...
   *
   * @param signatureKeyHandle the handle of a private key.
  ...
  public long findCertificateFromSignatureKeyHandle(long signatureKeyHandle) throws
    ...
    log.debug("Find certificate from signature key handle: "
        + signatureKeyHandle);
    ...
    pkcs11Module.C_GetAttributeValue(getSession(), signatureKeyHandle, attributeTemplateList, true);

    byte[] keyAndCertificateID = (byte[]) attributeTemplateList[0].pValue;
    log.debug("ID of signature key: "
   ...
   * read from the token.
   *
   * @param label the object label on the token.
   * @return the DER encoded certificate, as byte[]
   * @throws UnsupportedEncodingException
   * @throws TokenException
   */
  public byte[] getDEREncodedCertificateFromLabel(String label) throws TokenException {
  ...
  public byte[] getDEREncodedCertificate(long certHandle) throws
    ...
    pkcs11Module.C_GetAttributeValue(getSession(), certHandle, template, true);
  ...
  public byte[] getDEREncodedCertificate(long certHandle, long sessionHandle) throws
    ...
    template[0] = new CK_ATTRIBUTE();
    template[0].type = PKCS11Constants.CKA_VALUE;
    pkcs11Module.C_GetAttributeValue(sessionHandle, certHandle, template, true);
    certBytes = (byte[]) template[0].pValue;
   ...
   * @throws Throwable
   */
  public void moduleFinalize() throws Throwable {
    log.debug("finalizing PKCS11 module...");
   ...
   * {@link #login(char[])}.
   *
   * @param pwd password as a String.
   * @throws PKCS11Exception
   */
  ...
  public void login(char[] pwd) throws PKCS11Exception {
    ...
    pkcs11Module.C_Login(getSession(), PKCS11Constants.CKU_USER, pwd, true);
    log.debug("User logged into session.");
  }
  ...
  private void getModuleInfo() throws PKCS11Exception {
    log.debug("getting PKCS#11 module info");
    CK_INFO moduleInfo = pkcs11Module.C_GetInfo();
    log.debug(moduleInfo.toString());
  }
  ...
  public long[] getTokenList() {
    ...
    try {
      ...
      log.debug("PKCS11Exception: " + ex);
    }
    CK_TOKEN_INFO tokenInfo;
    log.debug(tokenIDs.length + " tokens found.");
    for (int i = 0; i < tokenIDs.length; i++) {
      log.debug(i + ") Info for token with handle: " + tokenIDs[i]);
      tokenInfo = null;
      try {
   ...
   *
   * @param mechanismCode the mechanism ID.
   * @param tokenID       the token handla.
  ...
  public boolean isMechanismSupportedByToken(long mechanismCode, long tokenID) throws
    ...
    log.debug("listing  mechanisms:");
    for (int i = 0; i < mechanismIDs.length; i++) {
      log.debug(mechanismIDs[i] + ": " + Functions.mechanismCodeToString(mechanismIDs[i]));
    }
   ...
   *
   * @param aTokenHandle the token ID.
   * @throws TokenException
   */
  public long openSession(long aTokenHandle) throws TokenException {
    ...
    log.debug("Session with handle: " + sessionHandle + " opened on token with handle: " + aTokenHandle);
  ...
  public void openSession() throws TokenException {
    long sessionHandle = -1L;
    if (getTokenHandle() >= 0) {
      sessionHandle = pkcs11Module.C_OpenSession(getTokenHandle(), PKCS11Constants.CKF_SERIAL_SESSION, null, null);
   ...
   *
   * @param newCryptokiLibrary the cryptoki name.
   */
   ...
   *
   * @param token the token handle to set.
   */
